...
from models.modeling_elasticbert_kernel import ElasticBertForSequenceClassification

from evaluations import evaluate_elue_kernel
from inferences import inference_elue_kernel
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results_all = evaluate_elue_kernel(args, model, tokenizer)
                        evg_metric_all = 0
                ...
                # if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0 and not args.not_save_model:
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        metric_key = get_metric_key(args.task_name)
        infer_threshold_list = [float(x) for x in args.infer_threshold.split(',')]
        ...
        if os.path.exists(best_model_path):
            ...
            model.to(args.device)
        for infer_threshold in infer_threshold_list:
            if args.debug:
                fitlog.debug()
            if args.local_rank in [-1, 0]:
                fitlog.set_log_dir(args.log_dir, new_log=True)
                fitlog.add_hyper(args)
                fitlog.add_hyper(value=infer_threshold, name='infer_threshold')

            best = 0.
            for checkpoint in checkpoints:
                if checkpoint.split("/")[-1] == "best_model":
                    global_step = 808
                    prefix = "best_model"
                else:
                ...
                result, speed_up, exit_layer_every_ins = evaluate_elue_kernel(args, model, tokenizer, prefix=prefix, eval_highway=True, infer_threshold=infer_threshold)
                res_for_display = {}
                ...
                if result[metric_key] > best:
                    ...
                    output_eval_file = os.path.join(args.output_dir, "{}_{}_{}_{}.tsv".format(args.task_name, 'eval', 'exit_layer', str(infer_threshold)))
                    ...
                    infer_speed_up = inference_elue_kernel(args, model, tokenizer, prefix=prefix, eval_highway=True, infer_threshold=infer_threshold)
