...
def inference_elue_kernel(args, model, tokenizer, prefix="", eval_highway=False, infer_threshold=0.):
    model.elasticbert.set_infer_threshold(infer_threshold)
    ...
    output_infer_file = os.path.join(eval_output_dir, "{}_{}.tsv".format(eval_task, str(infer_threshold)))
    ...
    output_infer_file = os.path.join(eval_output_dir, "{}_{}_{}_{}.tsv".format(eval_task, 'infer', 'exit_layer', str(infer_threshold)))
    with open(output_infer_file, "w", encoding='utf-8') as fout:
        writer = csv.writer(fout, delimiter='\t', quotechar=None)
...
def inference_glue_kernel(args, model, tokenizer, prefix="", eval_highway=False, infer_threshold=0.):
    model.elasticbert.set_infer_threshold(infer_threshold)
    model.elasticbert.set_eval_state(eval_highway)
    model.elasticbert.reset_stats()
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        exiting_layer_every_ins = model.elasticbert.exiting_layer_every_ins
        output_infer_file = os.path.join(eval_output_dir,
                                         "{}_{}_{}_{}.tsv".format(eval_task, 'infer', 'exit_layer', str(infer_threshold)))
        with open(output_infer_file, "w", encoding='utf-8') as fout:
            writer = csv.writer(fout, delimiter='\t', quotechar=None)
            writer.writerow(["index", "exit_layer"])
            for i, exit_layer in enumerate(exiting_layer_every_ins):
